...
for (i in (1:nrow(arguments))) { #i = 1
  ...
  code <- stringr::str_replace_all(string = code, pattern = "%categoryWebApi%", replacement = argument$categoryAsUsedInWebApi)
  code <- stringr::str_replace_all(string = code, pattern = "%categoryUrlGeneration%", replacement = argument$categoryUrlGeneration)
