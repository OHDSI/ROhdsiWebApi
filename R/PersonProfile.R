...
#'                       If left NULL, the cohortId will identify the earliest cohort for the person , by 
#'                       cohort start date and use it as the index cohort. 
#'                       WebApi uses the cohort start date of the index cohort to calculate the person's 
#'                       index age (ageAtIndex). WebApi will also return the relative position, in days, 
#'                       for each event compared to the index cohorts start date. These relative positions
#'                       are useful to study the relationship of various events with respect to the index
#'                       cohort start date.          
#' @return               A list of tibble data frame objects corresponding to cohorts, observationPeriod,
#'                       records and person.
#' @examples
#' \dontrun{#'
...
getPersonProfile <- function(baseUrl,
  ...
  #   checkmate::assertInt(indexCohortId, add = errorMessage)  -- fails for big integer. TODO
  # }
  ...
  data$records$endDate <- dplyr::case_when(is.na(data$records$endDate) ~ 
                                             lubridate::as_datetime(data$records$startDate +
                                                                      (data$records$endDay - 
                                                                         data$records$startDay
                                                                      )
                                             ),
