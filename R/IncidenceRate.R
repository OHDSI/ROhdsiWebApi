...
#' @return
#' A list with incident rate results summary, stratified results and treemap data.
#'
#' @examples
#' \dontrun{
...
getIncidenceRateResults <- function(incidenceRateId,
  ...
  if (nrow(data$stratifyStats) > 0) {
    data$stratifyStats$proportionP1K <- (data$stratifyStats$cases/data$stratifyStats$totalPersons) *
      1000
    ...
    data$stratifyStats$proportionP1K <- 0
    data$stratifyStats$rateP1K <- 0
    data$stratifyStats$sourceKey <- sourceKey
    data$stratifyStats$incidenceRateId <- incidenceRateId
  ...
  treeMapResultDf <- treeMapResultDf %>% dplyr::mutate(SatisfiedNumber = stringr::str_count(string = treeMapResultDf$bits,
                                                                                            pattern = "1"),
                                                       SatisfiedRules = stringr::str_locate_all(string = treeMapResultDf$bits,
                                                                                                pattern = "1") %>%
    paste())
