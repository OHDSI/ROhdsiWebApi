...
#'
#' @details
#' Constructs a CohortDefinition set containing the following fields: \describe{ \item{atlasId}{The
#' cohort ID in ATLAS.} \item{cohortId}{a copy of the value in atlasId.} \item{cohortName}{The name of
#' the cohort.} \item{sql}{The cohort generation sql.} \item{json}{The cohort definition JSON.}
...
exportCohortDefinitionSet <- function(baseUrl, cohortIds, generateStats = FALSE) {
  ...
  cohortDefinitionSet <- dplyr::tibble(atlasId = integer(),
    ...
    sql = character(), json = character(), logicDescription = character(), generateStats = logical())
  ...
  for (i in (1:length(cohortIds))) {
    ...
    cohortDefinitionSet <- dplyr::bind_rows(cohortDefinitionSet, dplyr::tibble(atlasId = cohortId,
      cohortId = cohortId, cohortName = object$name, sql = sql, json = json, logicDescription = ifelse(is.null(object$description),
        NA, object$description), generateStats = as.logical(generateStats)))
  }
