...
# Active Directory and database authentication
...
bearer token from WebAPI and saving it in the ROhdsiWebApi global package environment. If you are using Active Directory then set `authMethod = "ad"` and if you are using 
...
the [keyring package](https://cran.r-project.org/web/packages/keyring/readme/README.html). Never store passwords in your R code.
...
```{r, echo=FALSE}
print("https://atlas.odysseusinc.com/WebAPI")
```

```{r, eval=FALSE}
ROhdsiWebApi:::ROWebApiEnv[["https://atlas.odysseusinc.com/WebAPI"]]
```
